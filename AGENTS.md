...
- **2026-06-21 ミスマージ解消確認**: T29/T31 (旧 PR #18) は新 PR #19 (`main` ベースで再ランディング) が人間マージ済み。`gh` の表示のみに頼らず `origin/main` の生ログで squash コミット (#19) の到達を直接検証済み。fresh 全ゲート再検証 (pytest 365 / ruff / mypy strict 68 files) 緑。**残: 旧土台ブランチ `agent/T30-impl` (remote) の削除のみ未了**(内容は main 到達済み、削除は人間判断)。詳細は [docs/PROJECT_STATE.md](docs/PROJECT_STATE.md) 改訂履歴。
...
- `agent/T<N>-impl`: 各実装チケット (例: `agent/T5-impl`)。**コンフリクト防止のため、PR マージ後はブランチを使い回さず、必ず最新の `main` から次のブランチを切り直す。** 同一ブランチ内でのレビュー差し戻し修正は可。
