...
## 6. 人間判断待ち (着手前ブロッカー)
- **Irodori-TTS-Server の実行環境**: モデルカード想定は NVIDIA GPU (RTX 4070 Ti Super 級)。開発機は macOS — Apple Silicon でのサーバ稼働可否 / 別マシン / クラウド GPU の選択は**人間判断** (WORKFLOW §4 区分 D)。不可なら Kokoro 等の fallback エンジンで T24 を代替する判断も人間。
- **HAL 声リファレンス確定**: VoiceDesign キャプション合成 → 試聴 → Speaker Inversion 固定化の検証 (ADR-0006)。**試聴判断は人間**。
- **BGM/ジングル素材**: Lo-fi + 中華風アンビエント (FR-100)。入手元・ライセンス確認は人間。
- **mp3 配信方法**: Discord 添付 (25MB 以内) で開始か、最初から R2/S3 リンクか (要件 §17.6)。
- **A/B/C 比較の継続要否** (ADR-0005): T22 実測を見て既定 variant を確定。

## 7. 着手手順 (ゲート)
1. **T22 完了** (3日観察、2026-06-14 予定) → **Sprint 1B 完了 PR を人間がマージ** → **人間が Sprint 2 Go を宣言** (PROJECT_STATE に記録)。
2. §6 のうち「実行環境」「声リファレンス」を解消 (他はモック駆動で並行可、1B の T13 方式)。
