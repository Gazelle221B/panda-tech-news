...
| 2026-06-17 | Claude Code | **PR #16 (Irodori アダプタ) Copilot 計3件対応・全 resolved**。(1) mypy override を `kokoro_onnx.*`→`kokoro_onnx` (トップレベル import 未カバーの実バグ、no-extra 隔離 worktree で mypy Success 実証) (2) Irodori エラーに HTTP status code 追加 (秘密非記載) (3) `_post_with_retry` で最大2回リトライ (FR-013、既存 llm/fetcher と同 idiom)。Codex がレビュー中の T30 主作業ツリーを汚さぬよう **git worktree で隔離処理**。#16 は Codex PASS + QA PASS + Copilot 全 resolved で人間マージ待ち |
