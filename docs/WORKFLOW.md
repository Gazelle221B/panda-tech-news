# マルチエージェント開発ワークフロー v1.0.1(完全版)

最終更新: 2026-05-29 設計根拠: Terminal-Bench 2.1(2026-05-01)および SWE-bench 系統、ならびに英語圏/中国語圏コミュニティの運用観測 ステータス: 思想・ロール・フロー・I/O契約・権限管理・状態永続化・証跡管理を確定。運用投入可能
...
本配置は2026-05時点のベンチ(Terminal-Bench 2.1でCodex CLI/GPT-5.5が83.4%で首位、Claude Code/Opus 4.7が69.7%)と運用観測に基づく。モデル更新で各エージェントの得意領域は入れ替わりうるため、新モデルのリリース時に再評価する。特に「実装主軸=OpenCode、レビュー=Codex、上流=Opus」の割り当ては現時点の順位に依存する。
