...
依存: `stable_audio_3` は optional extra `sfx` (`uv sync --extra sfx`, torchaudio/soundfile
込み)。PyPI に公開が無いため git 依存 (`pyproject.toml` [tool.uv.sources] 参照)。torch を
含む重量級のため未導入でも本体パイプライン (produce 等) には一切影響しない (遅延 import)。
...
実機確認 (2026-08-01, CPU): モデルロード ~32秒、2.5〜4秒クリップの生成は1本あたり4〜6秒
(既定 8 diffusion steps)。ロード時に stderr へ出る `flash_attn not installed, disabling
Flash Attention` は Windows/CPU 環境で必ず出る無害な警告で、動作・生成結果には影響しない
(例外ではないため `_load_model` の GenSfxError 変換には掛からない)。
...
DEFAULT_SEED = 0
# "small-sfx" は StableAudioModel.from_pretrained() が要求するエイリアス。フル repo id
# ("stabilityai/stable-audio-3-small-sfx") を渡すと ValueError になる (実機確認 2026-08-01。
# 有効な値は ['small-music', 'small-music-base', 'small-sfx', 'small-sfx-base', 'medium',
# 'medium-base', 'same-s', 'same-l'])。
MODEL_ID = "small-sfx"

# kind ごとの既定プロンプト (英語, 放送向け短尺 SFX を狙う。README の Use case
# 「UI・トランジション」を踏まえた文言)。`--prompt` で個別に上書き可能。
# transition は実機検証 (2026-08-01) の知見で更新: ドライな打楽器系スティンガー
# (マリンバ/シロフォン) が良い当たりを出す一方、whoosh/持続音系はノイズ化しやすかった。
_DEFAULT_PROMPTS: dict[str, str] = {
    "transition": (
        "short news stinger, two quick bright marimba notes, clean studio recording, "
        "dry, no whoosh, no noise"
    ),
    "opening": (
...
def generate_candidates(
    *,
    ...
    実機確認 (2026-08-01): `model.generate()` は `torch.Tensor` (stereo, 44100Hz) を返す。
    batch 次元 (dim==3) が付く場合があるため先頭を取り、float32 化した上でピーク振幅が
    1.0 を超える場合のみスケールダウンする (torchaudio.save のクリップ防止)。
    """
    import torch
    import torchaudio
    ...
    for i in range(count):
        ...
        audio: torch.Tensor = model.generate(  # type: ignore[attr-defined]
            prompt=prompt, duration=duration, seed=candidate_seed
        )
        if audio.dim() == 3:
            audio = audio[0]
